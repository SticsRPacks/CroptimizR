...
param_info <- list(init_values=data.frame(dlaimax=c(0.001, NA),
                                          durvieF=c(75, 100)),
...
test_that("Initial values and constraints", {
  res1 <- eval(parse(text = "CroptimizR:::complete_init_values(param_info,
  nb_values=5,ranseed=1234)"))
  res2 <- eval(parse(text = "CroptimizR:::complete_init_values(param_info,
  nb_values=5,ranseed=4321)"))
  res3 <- eval(parse(text = "CroptimizR:::complete_init_values(param_info,
  nb_values=100,satisfy_par_const=satisfy_par_const,ranseed=1234)"))
...
# Simple case 2
param_info <- list(init_values=data.frame(dlaimax=c(0.001,0.002),
                                          durvieF=c(50,200)),
                lb=c(dlaimax=0.0001, durvieF=50),
                ub=c(dlaimax=0.01, durvieF=400))
eval(parse(text = "init_values <- CroptimizR:::get_init_values(param_info)"))
...
param_info <- list()
param_info$dlaimax <- list(sit_list=list(c("bou99t3", "bou00t3", "bou99t1",
                                           "bou00t1", "bo96iN+", "lu96iN+",
                                           "lu96iN6", "lu97iN+")),
                        init_values=0.001,lb=0.0001,ub=0.1)
param_info$durvieF <- list(sit_list=list(c("bo96iN+", "lu96iN+", "lu96iN6",
                                           "lu97iN+"),
                                      c("bou99t3", "bou00t3", "bou99t1",
                                        "bou00t1")),
...
param_info$durvieF <- list(sit_list=list(c("bo96iN+", "lu96iN+",
                                           "lu96iN6", "lu97iN+"),
                                      c("bou99t3", "bou00t3",
                                        "bou99t1", "bou00t1")),
