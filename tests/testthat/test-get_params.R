...
test_that("get_params_bounds", {
  expect_equal(
    eval(parse(text = "CroptimizR:::get_params_bounds(prior_simple)")),
    list(
      lb = c(dlaimax = 5e-04, durvieF = 5e+01),
      ub = c(dlaimax = 2.5e-03, durvieF = 4.0e+02)
    )
  )
  expect_equal(
    eval(parse(text = "CroptimizR:::get_params_bounds(prior_spec_var)")),
    list(
      lb = c(dlaimax = 5e-04, durvieF1 = 5e+01, durvieF2 = 1e+02),
...
prior_2 <- list(
  init_values = data.frame(
    dlaimax = c(0.001, 0.002),
    durvieF = c(50, 200)
  ),
  lb = c(dlaimax = 0.0001, durvieF = 50),
  ub = c(dlaimax = 0.01, durvieF = 400)
...
test_that("get_init_values", {
  expect_equal(
    eval(parse(text = "CroptimizR:::get_init_values(prior_1)")),
    data.frame(dlaimax = 1e-03, durvieF = 2e+02)
  )
  expect_equal(
    eval(parse(text = "CroptimizR:::get_init_values(prior_2)")),
    data.frame(dlaimax = c(0.001, 0.002), durvieF = c(50, 200))
  )
  expect_equal(
    eval(parse(text = "CroptimizR:::get_init_values(prior_3)")),
    data.frame(
      dlaimax = c(0.001, 0.002), durvieF1 = c(200, 300),
      durvieF2 = c(250, 350)
    )
  )
  expect_equal(
    eval(parse(text = "CroptimizR:::get_init_values(prior_4)")),
    data.frame(dlaimax = 1e-03)
  )
  expect_equal(
    eval(parse(text = "CroptimizR:::get_init_values(prior_5)")),
...
test_that("get_params_names", {
  expect_equal(
    eval(parse(text = "CroptimizR:::get_params_names(prior_1)")),
    c("dlaimax", "durvieF")
  )
  expect_equal(
    eval(parse(text = "CroptimizR:::get_params_names(prior_2)")),
  ...
  expect_equal(
    eval(parse(text = "CroptimizR:::get_params_names(prior_3)")),
