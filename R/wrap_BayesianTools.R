...
wrap_BayesianTools <- function(optim_options, param_info, crit_options) {
  ...
  if (is.null((optim_options$startValue))) {
  ...
  if (nargs() == 1 & methods::hasArg(optim_options)) {
    return(list(
      package = "BayesianTools", family = "Bayesian",
  ...
  sapply(names(default), function(x) {
    if (is.null(optim_options[[x]])) {
  ...
  if (method == "DREAMzs") {
    ...
    if (optim_options$burnin < optim_options$adaptation) {
      warning(paste0(
        "burnin (=", optim_options$burnin, ") < adaptation (=",
  ...
  likelihood <- function(x) {
    return(main_crit(x, crit_options))
  }
  ...
  out <- runMCMC(
    bayesianSetup = bayesianSetup, sampler = method,
