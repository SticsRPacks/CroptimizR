...
wrap_optim <- function(optim_options, param_info, crit_options) {
  ...
  if (is.null((hessian <- optim_options$hessian))) {
    hessian <- FALSE
  }
  if (is.null((method <- optim_options$method))) {
    method <- "Nelder-Mead"
  ...
  if (nargs() == 1 & methods::hasArg(optim_options)) {
    return(list(
      package = "optim", family = "Frequentist",
  ...
  optim <- lapply(optim, function(x) {
    x <- list(value = NA, par = rep(NA, nb_params))
  })
  ...
  for (irep in 1:nb_rep) {
    ...
    if (method == "L-BFGS-B" || method == "Brent") {
      try(optim[[irep]] <- stats::optim(
        par = as.numeric(init_values[irep, ]),
        fn = main_crit,
        method = method,
