...
#' A named list containing for each parameter the list of situations per group (\code{sit_list})
#' @param situation The name of the situation
#' @param param_vec The (named) vector of parameters from which to extract values
#'
#' @details Names of parameters in \code{param_vec} and \code{sit_groups} should
#' be the same. If \code{param_vec} is not named, the parameters in \code{param_vec}
#' will be considered in the same order as in \code{sit_group}
#'
#' @return A subvector of \code{param_vec} containing only the values that are
...
#' vec=c(1,2,3)
#' get_params_per_sit(sg,"sit2",vec)   # should give c(1,3)
#' get_params_per_sit(sg,"sit4",vec)   # should give c(2,3)
#' names(vec)=c("p2","p1","p1")
#' get_params_per_sit(sg,"sit2",vec)   # should give c(1,2)
#' get_params_per_sit(sg,"sit4",vec)   # should give c(1,3)
#'
