...
params:
  ...
  path_to_JavaStics: "D:/Home/sbuis/Documents/GitHub/CroptimizR/vignettes/JavaSTICS-1.41-stics-9.0"
...
The `estim_param` function returns a list which is also stored in the optim_results.Rdata file of the `optim_options$path_results` folder. This list contains different information depending on the method used. 
...
Let's print it for repetition 2: 
...
```{r eval=params$eval_auto_vignette, echo=FALSE, message=FALSE, warning=FALSE, out.width = '80%', fig.cap="Figure 2: plots of simulated and observed target variable before and after optimization. The gap between simulated and observed values has been drastically reduced: the minimizer has done its job!"}
...
file.copy(file.path(optim_options$path_results,"EstimatedVSinit.pdf"),params$result_path, overwrite = TRUE)
file.copy(file.path(optim_options$path_results,"optim_results.Rdata"),params$result_path, overwrite = TRUE)
file.copy(file.path(optim_options$path_results,"sim_obs_plots.png"),params$result_path, overwrite = TRUE)
```
