...
model_wrapper <- function( param_values=NULL, situation, model_options, ...) {
  ...
  results <- list(sim_list = setNames(vector("list", length(situation)), 
                                      nm = situation),
                 error = FALSE)
  attr(results$sim_list, "class") <- "cropr_simulation"
  ...
  for (situation in situation) {
      ...
      # read the results and store the data.frame in
      # result$sim_list[[situation]]
...
lai_toymodel <- function(year, max_lai = 8, julday_incslope = 100, 
  ...
  end_day <- format(as.Date(paste0(year+1,"-12-31"), format = "%Y-%m-%d", 
                            origin=paste0(year,"-01-01")), "%j")
...
tmp <- laitm_simple_wrapper(situation = "2005_a", 
                            param_values = c(inc_slope = 25, dec_slope = 10))
...
param_info <- list(lb = c(inc_slope = 1, dec_slope = 1), 
...
tmp <- laitm_simple_wrapper(situation=c("2005_a", "2006_b"),
...
tmp <- laitm_simple_wrapper_v2(situation = c("2005_a","2006_b"),
                            param_values = dplyr::tibble(situation =c("2005_a",
                                                                      "2006_b"),
                                                  inc_slope = c(25,50),
                                                  dec_slope = c(10,10)))
...
obs_synth <- list(`2005_a`=tmp$sim_list$`2005_a`[seq(from=1, to=length_2005_a,
                                                     by=3),],
                  `2006_b`=tmp$sim_list$`2006_b`[seq(from=1, to=length_2006_b,
                                                     by=3),])
